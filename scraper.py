...
# Proxy setup (to be replaced with Bright Data)
...
BUCKET_NAME = "watch-arbitrage-bucket"  # Update with actual bucket name
storage_client = storage.Client()
bucket = storage_client.bucket(BUCKET_NAME)

def save_to_gcs(filename, data):
...
def update_logs(log_entry):
    try:
        logs = load_from_gcs("scraper_log.csv")
    except:
    ...
    save_to_gcs("scraper_log.csv", logs)
...
FAKE_MODEL = None  # Load via Fiverr: tf.keras.models.load_model('watch_authenticity_model.h5')

def detect_fake(row):
    if 'no serial' in row['Description'].lower() or 'replica' in row['Description'].lower() or row['Price'] < 100:
        return 'Likely Fake'
    market_prices = {'Rolex Submariner': 8000, 'Seiko Prospex': 400, 'Omega Speedmaster': 3000}  # Replace with real data
    key = f"{row['Brand']} {row['Model']}"
    if key in market_prices and row['Price'] < market_prices[key] * 0.3:
        return 'Likely Fake'
    if 'Image URL' in row and row['Image URL'] and FAKE_MODEL:
        img = fetch_image(row['Image URL'])
        if img:
            img = img.resize((224, 224)).convert('RGB')
            pred = FAKE_MODEL.predict(np.array([img]))[0][0]
            return 'Likely Fake' if pred > 0.5 else 'Likely Genuine'
    ...
    try:
        ...
        for item in soup.select('.article-item'):
            try:
                ...
                brand, model = parse_title(title)
                listings.append({
                    'Date Listed': datetime.now().strftime('%Y-%m-%d'),  # Chrono24 doesn’t show exact date
                    'Platform': 'Chrono24',
                    'Brand': brand,
                    'Model': model,
                    'Price': price,
...
# Implement remaining 14 functions similarly, adjusting selectors per site
def scrape_sothebys(max_price):
    ...
    try:
        response = requests.get(url, headers=HEADERS, proxies=get_proxy(), timeout=10)
        soup = BeautifulSoup(response.text, 'html.parser')
        for item in soup.select('.lot-item'):  # Adjust selector
            try:
                price_str = item.select_one('.price').text.replace('$', '').replace(',', '')
                price = float(price_str)
                if price > max_price:
...
def scrape_christies(max_price):
    ...
    try:
        response = requests.get(url, headers=HEADERS, proxies=get_proxy(), timeout=10)
        soup = BeautifulSoup(response.text, 'html.parser')
        for item in soup.select('.lot'):  # Adjust selector
            try:
...
def scrape_watchuseek(max_price):
    ...
    try:
        response = requests.get(url, headers=HEADERS, proxies=get_proxy(), timeout=10)
        soup = BeautifulSoup(response.text, 'html.parser')
        for item in soup.select('.thread'):  # Adjust selector
            try:
...
def scrape_reddit(max_price):
    ...
    try:
        response = requests.get(url, headers=HEADERS, proxies=get_proxy(), timeout=10)
        soup = BeautifulSoup(response.text, 'html.parser')
        for item in soup.select('.Post'):  # Adjust selector
            try:
                price_str = item.select_one('.price').text.replace('$', '').replace(',', '') if item.select_one('.price') else '0'
                price = float(price_str)
                if price > max_price or price == 0:
...
def scrape_catawiki(max_price):
    ...
    try:
        ...
        for item in soup.select('.lot'):  # Adjust selector
            try:
                price_str = item.select_one('.lot-price').text.replace('€', '').replace(',', '').strip()  # Assuming EUR
...
def scrape_timepeaks(max_price):
    ...
    try:
        response = requests.get(url, headers=HEADERS, proxies=get_proxy(), timeout=10)
        soup = BeautifulSoup(response.text, 'html.parser')
        for item in soup.select('.item'):  # Adjust selector
            try:
                price_str = item.select_one('.price').text.replace('$', '').replace(',', '')
                price = float(price_str)
                if price > max_price:
...
def scrape_bobs_watches(max_price):
    ...
    try:
        response = requests.get(url, headers=HEADERS, proxies=get_proxy(), timeout=10)
        soup = BeautifulSoup(response.text, 'html.parser')
        for item in soup.select('.auction-item'):  # Adjust selector
            try:
                price_str = item.select_one('.price').text.replace('$', '').replace(',', '')
                price = float(price_str)
                if price > max_price:
