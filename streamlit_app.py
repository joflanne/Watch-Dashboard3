...
def load_data():
    ...
    data = load_from_file(DATA_FILE, ['Date Listed', 'Platform', 'Brand', 'Model', 'Price', 'Listing URL', 'Image URL', 'Description'])
    
    # If no data or file is empty, scrape fresh data
    if data.empty:
        data_sources = [
...
data = load_data()  # No caching for now
logs = load_logs()
...
with tab1:
    ...
    if data.empty:
        st.warning("No inventory data available yet. Check scraper logs for activity.")
    else:
